...
int main(int argc, char *argv[])
{
    ...
    VideoCapture cap;
    cap.open(vidPath);
    std::cout << "create capture using videos!" << std::endl;
    int delay = 1;
    int frame_id = 0;
    while (1)
    {
        ...
        if (img.empty())
        {
            cout << "Cap source Empty!";
            break;
        }
        string imgs_name = "video_input";
        vector<Mat> imgs;
        vector<string> imgs_names;
        imgs.push_back(img);
        imgs_names.push_back(imgs_name);
        vector<int> input_cols, input_rows;
        vector<Mat> inputs;
        // std::cout << "getting video img success!" << std::endl;
        for (long unsigned int i = 0, j = -1; i < imgs.size(); i++)
        {
            ...
            for (int k = 0; k < static_cast<int>(inputs.size()); k++)
            {
                ...
                for (auto &box : results[k].bboxes)
                {
                    ...
                    if (ymin < 0.)
                        ymin = 1.;
                    if (xmax > input_cols[k])
                        xmax = input_cols[k];
                    if (ymax > input_rows[k])
                        ymax = input_rows[k];
                    ...
                    bbox.x1 = int(xmin);
                    bbox.y1 = int(ymin);
                    bbox.w = int(xmax - xmin);
                    bbox.h = int(ymax - ymin);
            ...
            for (TrackingBox it : tracking_results)
            {
                ...
                rectangle(imgs[0], object, tracker.randColor[it.id % 255], 2);
                putText(imgs[0],
                        to_string(it.id),
